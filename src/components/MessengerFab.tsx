...
export default function MessengerFab() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] flex flex-col items-end gap-3">
      ...
      {/* Tooltip */}
      <div
        className={`pointer-events-none mr-[72px] transition-all duration-300 ${
      ...
      <div className="relative">
        <span className="absolute inset-0 rounded-full bg-brand-500/70 animate-ring" aria-hidden="true" />
        <span
          className="absolute inset-0 rounded-full bg-brand-400/50 animate-ring [animation-delay:0.9s]"
          aria-hidden="true"
        />
        <button
          onClick={() => setOpen((v) => !v)}
